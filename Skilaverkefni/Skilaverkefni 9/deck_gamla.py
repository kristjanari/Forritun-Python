...
class Card:
    ...
    def __init__(self, rank = 0, suit = ""):
        self.rank_dict = {"A": 1, "J": 11, "Q": 12, "K": 13}
        self.rank_dict2 = {1: "A", 11: "J", 12: "Q", 13: "K"}
        if type(rank) == str:
            try:
    ...
    def __str__(self): 
        if self.is_blank():
            return "blk"
        elif self.rank in self.rank_dict2:
            return "{:>3}".format(self.rank_dict2[self.rank] + self.suit)
        else:
...
class Deck:
    ...
    def __init__(self):
        ...
        for i in range(1,14):
            for a_suit in suits:
                if i in rank_dict2:
                    deck.append(rank_dict2[i] + a_suit)
                else:
                    deck.append(str(i) + a_suit)
        self.deck = deck
    ...
    def __str__(self):
        ...
        for k in range(4):
            for i in range(13):
                try:
                    deck_string  += "{:>4}".format(self.deck[k*13+i])
                except:
                    pass
            deck_string += "\n"
        return deck_string
